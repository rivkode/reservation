...
/**
 * Kafka consumer 설정. common-infrastructure 는 Outbox relay producer 만 제공하므로
 * consumer 측 factory 는 각 서비스가 자체 정의한다 (Phase 1 컨벤션).
 *
 * <p>payload 를 {@code byte[]} 로 받아 {@code HotelEventsKafkaConsumer} 에서 event-type
 * 헤더 기반 디스패치 후 수동 역직렬화한다. 이 방식은 contracts 이벤트 record 에
 * {@code @JsonTypeInfo} 등 다형 역직렬화 메타를 두지 않아도 되게 한다.
 *
...
public class KafkaConsumerConfig {
    ...
    @Bean
    public ConsumerFactory<String, byte[]> hotelEventsConsumerFactory(
        @Value("${spring.kafka.bootstrap-servers}") String bootstrapServers,
        @Value("${app.kafka.consumer.hotel-events.group-id:reservation-service.hotel-events}") String groupId) {
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, byte[]> hotelEventsListenerContainerFactory(
        ConsumerFactory<String, byte[]> hotelEventsConsumerFactory) {
