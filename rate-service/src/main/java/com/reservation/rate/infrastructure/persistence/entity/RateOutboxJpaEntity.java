...
/**
 * rate-service 로컬 Outbox 테이블 매핑. {@code rate_outbox} 테이블 한 건당 한 개의
 * Kafka 메시지를 예약한다 (ADR 0003). 스키마 · 인덱스는 V1 Flyway 마이그레이션 참고.
 */
@Entity
@Table(name = "rate_outbox")
public class RateOutboxJpaEntity {
    ...
    protected RateOutboxJpaEntity() {
    }

    public RateOutboxJpaEntity(UUID id, String topic, String eventType, UUID eventId,
